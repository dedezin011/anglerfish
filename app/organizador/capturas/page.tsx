...
import { updateOrganizerCaptureReview } from "@/app/organizador/capturas/actions";
import { getOrganizerSession } from "@/lib/organizer-auth";
import { getSupabaseAdmin } from "@/lib/supabase";

type CaptureStatus = "pending" | "approved" | "rejected";

type OrganizerCapturasPageProps = {
...
export const metadata = {
  title: "Capturas do Organizador | AnglerFish",
  description: "Revisão de capturas por torneio para organizadores AnglerFish.",
  robots: {
    index: false,
    follow: false
  }
...
async function loadOrganizerCaptures({
  userId,
  ...
  const tournamentIds = [
    ...new Set(((linksData ?? []) as { tournament_id: string }[]).map((link) => link.tournament_id))
  ];

  if (!tournamentIds.length) {
    return {
      tournaments: [] as TournamentRow[],
      selectedTournament: null as TournamentRow | null,
      captures: [] as CaptureView[],
      allStatusRows: [] as CaptureRow[]
    };
  }
...
function ReviewForm({
  ...
  return (
    <form action={updateOrganizerCaptureReview} className="grid gap-3">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-slate-500">
            {capture.tournament?.name ?? "Torneio não encontrado"} · código{" "}
            {capture.tournament?.code ?? "-"}
          </p>
        </div>
...
export default async function OrganizerCapturasPage({
  searchParams
}: OrganizerCapturasPageProps) {
  ...
  const { tournaments, selectedTournament, captures, allStatusRows } =
    await loadOrganizerCaptures({
      userId: session.user.id,
      requestedTournamentId: params.tournament,
      status
    });

  const selectedTournamentId = selectedTournament?.id ?? "";
  const redirectTo = `/organizador/capturas?tournament=${selectedTournamentId}&status=${status}`;
  ...
  return (
    <main className="min-h-screen bg-foam text-midnight">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-5 sm:px-8 md:flex-row md:items-center md:justify-between">
          <Link href="/organizador" className="flex items-center gap-3 font-bold">
            ...
            />
            <span className="hidden text-sm text-slate-500 sm:inline">Capturas</span>
          </Link>
      ...
      <section className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
        <div className="mb-8">
          <p className="text-sm font-black uppercase text-reef">Revisão por campeonato</p>
          <h1 className="mt-2 text-3xl font-black sm:text-4xl">Capturas do torneio</h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
            Este painel mostra apenas torneios vinculados à sua conta de organizador.
          </p>
        </div>
        ...
        {selectedTournament ? (
          <>
            <div className="mb-5 flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-xs font-black uppercase text-slate-500">Torneio selecionado</p>
                <h2 className="mt-1 text-xl font-black text-midnight">{selectedTournament.name}</h2>
                <p className="mt-1 text-sm font-semibold text-slate-500">
                  Código {selectedTournament.code}
                </p>
              </div>
              <nav className="flex flex-wrap gap-2" aria-label="Alternar torneio">
                {tournaments.map((tournament) => {
                  const active = tournament.id === selectedTournament.id;

                  return (
                    <Link
                      key={tournament.id}
                      href={`/organizador/capturas?tournament=${tournament.id}&status=${status}`}
            ...
            <nav className="mt-6 flex flex-wrap gap-2" aria-label="Filtros de status">
              {statusFilters.map((filter) => {
                ...
                return (
                  <Link
                    key={filter.value}
                    href={`/organizador/capturas?tournament=${selectedTournament.id}&status=${filter.value}`}
                    className={`inline-flex min-h-10 items-center justify-center rounded-md px-4 py-2 text-sm font-black transition ${
                      active
                        ? "bg-midnight text-white"
                        : "border border-slate-200 bg-white text-midnight hover:bg-foam"
            ...
            <div className="mt-6 grid gap-5">
              {captures.length ? (
                ...
                <article className="rounded-lg border border-slate-200 bg-white p-8 text-center shadow-sm">
                  ...
                  <h2 className="mt-4 text-xl font-black text-midnight">
                    Nenhuma captura neste filtro
                  </h2>
                  <p className="mt-2 text-sm text-slate-500">
                    Quando pescadores enviarem capturas para este torneio, elas aparecerão aqui.
                  </p>
                </article>
              )}
            </div>
          ...
          <article className="rounded-lg border border-slate-200 bg-white p-8 text-center shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Vincule esta conta a um torneio no Supabase para liberar a revisão.
            </p>
          </article>
        )}
      </section>
